...
def FileToMongoEvent():
    ...
    @task.python
    def get_file_path_event(**context):
        try:
    ...
    def create_item_event(**context):
        try:
            status = context['task_instance'].xcom_pull(key="status", task_ids="get_file_path_event")
            if status == 1:
                raise Exception("previous task status is 1")

            xcom = context['task_instance'].xcom_pull(key="result", task_ids="get_file_path_event")
    ...
    def update_meta_event(**context):
        try:
            status = context['task_instance'].xcom_pull(key="status", task_ids="create_item_event")
            if status == 1:
                raise Exception("previous task status is 1")
